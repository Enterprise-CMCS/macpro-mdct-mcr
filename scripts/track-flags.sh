...
(grep $GREP_FLAGS $EXCLUSIONS "useFlags().*" ./services --include=*.jsx --include=*.tsx --exclude=*.test.jsx --exclude=*.test.tsx || echo "# N/A") | sort >> $LD_LOG_FILE

echo "" >> $LD_LOG_FILE

echo "# LaunchDarkly flags in tests" >> $LD_LOG_FILE
(grep $GREP_FLAGS $EXCLUSIONS "mockLDFlags.set.*" ./services --include=*.test.jsx --include=*.test.tsx || echo "# N/A") | sort >> $LD_LOG_FILE
