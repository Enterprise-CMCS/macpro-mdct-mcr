...
const compareToTime = new Date("20 July 2025 00:00").toISOString();
...
function filterData(items) {
  return items.filter(
    (item) => new Date(item.lastAltered).toISOString() > compareToTime
  );
}
...
function filterS3Objects(bucketObjects) {
  return bucketObjects.filter(
    (obj) => new Date(obj.LastModified).toISOString() > compareToTime
  );
}
