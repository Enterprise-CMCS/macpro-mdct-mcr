...
const ChildForm = ({
  ...
  if (table) {
    ...
    const handleTableSubmit = (enteredData: AnyObject) => {
      ...
      const hasExceptions =
        standardCompliance[0]?.value === planComplianceStandardExceptionsLabel;
...
const ParentForms = ({
  ...
  const { setChildFormId } = useContext(OverlayContext);
...
export const EntityDetailsMultiformOverlay = (props: Props) => {
  const { childFormId } = useContext(OverlayContext);
  ...
  return childFormId ? (
    <ChildForm
      ...
      validateOnRender={props.validateOnRender}
    />
  ) : (
    <ParentForms
      closeEntityDetailsOverlay={props.closeEntityDetailsOverlay}
      disabled={props.disabled}
      forms={props.forms}
      onSubmit={props.onSubmit}
      selectedEntity={props.selectedEntity}
      setEntering={props.setEntering}
      setSelectedEntity={props.setSelectedEntity}
      submitting={props.submitting}
      validateOnRender={props.validateOnRender}
