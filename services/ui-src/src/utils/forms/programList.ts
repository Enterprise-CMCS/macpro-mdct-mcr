...
// dynamically generate fields for program list
export const generateProgramList = (form: FormJson, state: string) => {
  const programListChoices = programList[state as keyof typeof programList];
  form.fields[0] = {
    ...form.fields[0],
    props: {
      ...form.fields[0].props,
      choices: [...updatedProgramListChoices(programListChoices)],
    },
  };
  return {
    ...form,
...
const updatedProgramListChoices = (choices: AnyObject[]) => {
  const updatedChoiceList: AnyObject[] = [];
  const programNameOtherText = {
    id: "d3oxrwNJCc7QFWHrS1LEg58b",
    label: "Other, specify",
    children: [
      {
  ...
  updatedChoiceList.push(programNameOtherText);
