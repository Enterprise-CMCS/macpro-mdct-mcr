describe("state user enters a program", () => {
  ...
  it("fills out B: State-Level Indicators", () => {
    ...
    cy.get('textarea[name="state_submittedDataAuditResults"]').type("https://www.auditwebsite.com");
    cy.findByRole("button", { name: "Save & continue" }).click();
  });
  ...
  it("fills out section C", () => {
    ...
    cy.get('input[name="program_contractUrl"]').type("https://www.contracturl.com");
    ...
    cy.get('textarea[name="program_specialBenefitsAvailabilityVariation"]').type("N/A");
    cy.get('input[name="program_enrollment"]').type("100");
    cy.get('textarea[name="program_enrollmentBenefitChanges"]').type("Changes to enrollment");
    cy.findByRole("button", { name: "Save & continue" }).click();  
    //III
    cy.get('input[name="program_encounterDataUses"]').get('[type="checkbox"]').check("Quality/performance measurement");
    cy.get('input[name="program_encounterDataUses"]').get('[type="checkbox"]').check("Timeliness of data corrections");
    cy.get('input[name="program_encounterDataSubmissionCorrectionPerformanceEvaluationCriteria"]').get('[type="checkbox"]').check("Timeliness of data certifications");
    cy.get('input[name="program_encounterDataSubmissionCorrectionPerformanceEvaluationCriteria"]').get('[type="checkbox"]').check("Use of correct file formats");
    cy.get('textarea[name="program_encounterDataSubmissionCorrectionPerformanceEvaluationCriteriaContractLanguageLocation"]')
      .type("References here");
    cy.get('textarea[name="program_encounterDataSubmissionQualityFinancialPenaltiesContractLanguageLocation"]')
      .type("Financial penalties");
    cy.get('textarea[name="program_encounterDataQualityIncentives"]')
      .type("N/A");
    cy.get('textarea[name="program_encounterDataCollectionValidationBarriers"]')
      .type("Barriers to collecting data");
    cy.findByRole("button", { name: "Save & continue" }).click();

    //IV
    cy.get('textarea[name="program_criticalIncidentDefinition"]').type("N/A");
    cy.get('textarea[name="program_standardAppealTimelyResolutionDefinition"]').type("No longer than 30 calendar days");
    cy.get('textarea[name="program_expeditedAppealTimelyResolutionDefinition"]').type("No longer than 72 hours");
    cy.get('textarea[name="program_grievanceTimelyResolutionDefinition"]').type("Within 90 days");
    cy.findByRole("button", { name: "Save & continue" }).click();

    //V
    cy.get('textarea[name="program_networkAdequacyChallenges"]').type("Challenges here");
    cy.get('textarea[name="program_networkAdequacyGapResponseEfforts"]').type("Addressing the gaps");
    cy.findByRole("button", { name: "Save & continue" }).click(); 
    cy.findByRole("button", { name: "Add access measure" }).click();
    cy.get('[type="radio"]').check("General quantitative availability and accessibility standard");
    cy.get('textarea[name="accessMeasure_standardDescription"]').type("Wait times");
    ...
    cy.get('textarea[name="state_bssEntityServiceAccessibility"]').type("In person, phone, email");
    cy.get('textarea[name="state_bssEntityLtssProgramDataIssueAssistance"]').type("Assistance here");
    cy.get('textarea[name="state_bssEntityPerformanceEvaluationMethods"]').type("Performance evaluations");
    cy.findByRole("button", { name: "Save & continue" }).click();
